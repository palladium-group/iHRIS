  ...
  <v-card>
    ...
    <v-card-text style="height: 90vh">
      <iframe
        ...
        :src="
          `/dashboards/app/dashboards#/view/${$route.params.id}?embed=true&_a=(description:'',filters:!(('$state':(store:appState),meta:(alias:!n,disabled:!f,key:ihris-related-group.keyword,negate:!f,params:(query:'${selectedFacility}'),type:phrase),query:(match_phrase:(ihris-related-group:'${selectedFacility}')))))&hide-filter-bar=true`
        "
        height="100%"
        style="border: none;"
        width="100%"
      ...
      <iframe
        v-if="!getData.location && !selectedFacility"
        :src="
          '/dashboards/app/dashboards#/view/' + $route.params.id + '?embed=true'
        "
        height="100%"
        style="border: none;"
        width="100%"
...
export default {
  ...
  created() {
    ...
    fetch(
      "/config/getParameters?key=" +
        encodeURIComponent("defaults:fields:Location.partOf:initialValue"),
    )
